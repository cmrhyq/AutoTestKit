...
class TestEcExtensionsImageApi:
    ...
    @pytest.fixture(scope="class")
    def public_params(self, test_env) -> ImageApiPublicParams:
        """提取镜像接口测试所需的公共参数。"""
        return ImageApiPublicParams(
            repo_name=test_env.get("nginxRepoName", "kube_system/nfs/provisioner/v1"),
            project_name=test_env.get("nginxProjectName", "kube_system"),
        )
