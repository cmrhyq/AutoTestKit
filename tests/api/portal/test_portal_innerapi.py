...
转换自 JMeter 脚本: portal-innerapi.jmx
测试内容：门户InnerAPI服务的全部接口功能验证，包括：
- 用户/租户/角色/系统/应用 全量数据查询
- 字典数据、系统参数、License、版本信息查询
- 平台基本信息、开启模块、全局配置查询与修改
- 授权信息、站内消息
- 一级域/二级域查询
- 系统/应用 创建
- 环境/平面/单元/产品实例 全量查询与条件查询
- 菜单权限管理（新增/停用/启用/删除）
- 角色管理（创建/修改/删除）
- 用户管理（查询/创建/删除/租户绑定/角色绑定/解绑）
- API管理（列表查询/批量授权/批量解除授权）
"""
...
@allure.story("portal-innerapi接口测试")
class TestPortalInnerapi:
    """
    对应 JMeter 脚本: portal-innerapi.jmx
    ...
    def test_get_first_field_list(self, portal_inner_service, api_cache):
        ...
        with AllureHelper.step("缓存一级域数据，供后续测试使用"):
            ...
            # 提取 systemName 为 "test" 的 systemId（对应 JMX 中 JSON 提取器逻辑）
            first1 = None
            for item in data:
                if item.get("systemName") == "test":
                    first1 = item.get("systemId")
                    break
            api_cache.set("first1", first1)
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    def test_create_system(self, portal_inner_service, api_env, api_cache):
        first1 = api_cache.get("first1")
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    def test_get_system_full_data(self, portal_inner_service, api_env, api_cache):
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    def test_create_application(self, portal_inner_service, api_env, api_cache):
    ...
    @allure.severity(allure.severity_level.CRITICAL)
    def test_get_application_full_data(self, portal_inner_service, api_env, api_cache):
